...
class AutoAdCog(commands.Cog):
    ...
    async def delete(self, interaction: discord.Interaction, ad_id: str) -> None:
        if not interaction.guild:
            return
        config = await self.manager.get_config(interaction.guild.id)
        before = len(config["ads"])
        config["ads"] = [ad for ad in config["ads"] if ad["id"] != ad_id]
        await self.manager.save_config(interaction.guild.id, config)
        ok = len(config["ads"]) < before
    ...
    @autoad.command(name="preview", description="Preview an advertisement")
    @admin_only()
    async def preview(self, interaction: discord.Interaction, ad_id: str) -> None:
        if not interaction.guild:
            return
        ad = next((item for item in (await self.manager.get_config(interaction.guild.id))["ads"] if item["id"] == ad_id), None)
        if not ad:
            await interaction.response.send_message(embed=make_embed("Not found", f"No advertisement with ID `{ad_id}`.", color=ERROR_COLOR), ephemeral=True)
            return
        content, embed, files = self.manager.build_payload(ad)
        await interaction.response.send_message(content=content, embed=embed, files=files, ephemeral=True)
    ...
    async def channel(self, interaction: discord.Interaction, channel: discord.TextChannel, enabled: bool = True) -> None:
        ...
        await self.manager.save_config(interaction.guild.id, config)
        await interaction.response.send_message(embed=make_embed("Channels updated", f"{channel.mention} is {'enabled' if enabled else 'disabled'} for AutoAd.", color=SUCCESS_COLOR), ephemeral=True)
    ...
    async def _set_enabled(self, interaction: discord.Interaction, enabled: bool) -> None:
        ...
        await self.manager.save_config(interaction.guild.id, config)
        await interaction.response.send_message(embed=make_embed("AutoAd updated", f"AutoAd is now {'enabled' if enabled else 'disabled'}.", color=SUCCESS_COLOR), ephemeral=True)

    @autoad.command(name="mode", description="Set dispatch mode")
    @admin_only()
    @app_commands.choices(mode=[app_commands.Choice(name=m, value=m) for m in ["random", "rotate", "every_message", "message_count", "timer"]])
    async def mode(self, interaction: discord.Interaction, mode: app_commands.Choice[str]) -> None:
        ...
        await self.manager.save_config(interaction.guild.id, config)
        await interaction.response.send_message(embed=make_embed("Mode updated", f"Mode set to `{mode.value}`.", color=SUCCESS_COLOR), ephemeral=True)

    @autoad.command(name="count", description="Set message-count mode threshold")
    @admin_only()
    async def count(self, interaction: discord.Interaction, count: app_commands.Range[int, 1, 10000]) -> None:
        await self._set_number(interaction, "message_count", int(count), "Message count")

    @autoad.command(name="interval", description="Set timer mode interval in minutes")
    @admin_only()
    async def interval(self, interaction: discord.Interaction, minutes: app_commands.Range[int, 1, 10080]) -> None:
        await self._set_number(interaction, "interval_minutes", int(minutes), "Interval")

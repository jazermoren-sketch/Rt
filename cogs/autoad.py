...
class AutoAdCog(commands.Cog):
    ...
    def _timer_due(self, config: dict[str, Any]) -> bool:
        last = config.get("stats", {}).get("last_sent_at")
        if not last:
            return True
        try:
            last_sent_at = dt.datetime.fromisoformat(last)
        except ValueError:
            return True
        return (dt.datetime.now(dt.UTC) - last_sent_at).total_seconds() >= int(config.get("interval_minutes", 60)) * 60
    ...
    async def _set_number(self, interaction: discord.Interaction, key: str, value: int, label: str) -> None:
        ...
        if value < 1:
            await interaction.followup.send(
                embed=make_embed("Invalid value", f"{label} must be at least 1.", color=ERROR_COLOR),
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            embed=make_embed(f"{label} updated", f"Set to `{value}`.", color=SUCCESS_COLOR),
            ephemeral=True,
        )
    ...
    async def create(
        ...
        if not any([message, image_url, embed_title, embed_description, attachment]):
            await interaction.followup.send(
                embed=make_embed("Nothing to save", "Provide message text, an image URL, an embed field, or an attachment.", color=ERROR_COLOR),
                ephemeral=True,
            )
            return
        ad = await self.manager.create_ad(
        ...
        await interaction.followup.send(
            embed=make_embed("Advertisement created", f"Saved `{ad['name']}` with ID `{ad['id']}`.", color=SUCCESS_COLOR),
            ephemeral=True,
        )
    ...
    async def list_ads(self, interaction: discord.Interaction) -> None:
        ...
        for ad in config.get("ads", [])[:25]:
            embed.add_field(name=f"{ad.get('name', 'Untitled')} (`{ad.get('id', 'unknown')}`)", value=f"Sent: {ad.get('sent', 0)}", inline=False)
        if not embed.fields:
    ...
    @admin_only()
    async def channel(self, interaction: discord.Interaction, channel: discord.TextChannel, enabled: bool = True) -> None:
        if not interaction.guild:
            return
        await interaction.response.defer(ephemeral=True)
        config = await self.manager.get_config(interaction.guild.id)
        channels = set(map(int, config.get("channels", [])))
        channels.add(channel.id) if enabled else channels.discard(channel.id)
    ...
    @autoad.command(name="disable", description="Disable AutoAd")
    @admin_only()
    ...
    async def _set_enabled(self, interaction: discord.Interaction, enabled: bool) -> None:
        ...
        await self.manager.save_config(interaction.guild.id, config)
        await interaction.followup.send(embed=make_embed("AutoAd enabled" if enabled else "AutoAd disabled", f"AutoAd is now `{enabled}`.", color=SUCCESS_COLOR), ephemeral=True)

    @autoad.command(name="mode", description="Set AutoAd mode")
    @admin_only()
    async def mode(self, interaction: discord.Interaction, mode: str) -> None:
        ...
        if normalized not in VALID_MODES:
            await interaction.followup.send(embed=make_embed("Invalid mode", f"Choose one of: {', '.join(sorted(VALID_MODES))}.", color=ERROR_COLOR), ephemeral=True)
            return
        config = await self.manager.get_config(interaction.guild.id)
        config["mode"] = normalized
        await self.manager.save_config(interaction.guild.id, config)
        await interaction.followup.send(embed=make_embed("Mode updated", f"Mode set to `{normalized}`.", color=SUCCESS_COLOR), ephemeral=True)
    ...
    async def mode_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice[str]]:
        return [app_commands.Choice(name=mode, value=mode) for mode in sorted(VALID_MODES) if current.lower() in mode][:25]

    @autoad.command(name="count", description="Set message-count trigger threshold")
    @admin_only()
    async def count(self, interaction: discord.Interaction, count: int) -> None:
        await self._set_number(interaction, "message_count", count, "Message count")

    @autoad.command(name="interval", description="Set timer interval in minutes")
    @admin_only()
    async def interval(self, interaction: discord.Interaction, minutes: int) -> None:
        await self._set_number(interaction, "interval_minutes", minutes, "Timer interval")
    ...
    async def _show_config(self, interaction: discord.Interaction, *, include_stats: bool) -> None:
        ...
        await interaction.response.defer(ephemeral=True)
        config = await self.manager.get_config(interaction.guild.id)
        embed = make_embed("AutoAd Statistics" if include_stats else "AutoAd Status")
        ...
        embed.add_field(name="Interval", value=f"{config.get('interval_minutes', 60)} minutes", inline=True)
        embed.add_field(name="Message count", value=str(config.get("message_count", 25)), inline=True)
        ...
        if include_stats:
            ...
            embed.add_field(name="Last sent", value=str(stats.get("last_sent_at") or "Never"), inline=False)
    ...
    async def export(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.send(file=discord.File(io.BytesIO(payload), filename="autoad.json"), ephemeral=True)
    ...
    async def import_config(self, interaction: discord.Interaction, file: discord.Attachment) -> None:
        ...
        try:
            ...
            await interaction.followup.send(embed=make_embed("Invalid JSON", "Upload a valid AutoAd export.", color=ERROR_COLOR), ephemeral=True)
            return

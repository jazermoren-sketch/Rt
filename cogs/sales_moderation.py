...
class SalesModerationCog(commands.Cog):
    ...
    async def handle_warning_submit(
        ...
        db[user_id]["count"] += 1
        db[user_id]["reasons"].append(reason)
        ...
        embed.add_field(name="نص المنشور:", value=original_msg.content[:100] if original_msg.content else "بدون نص", inline=False)
        ...
        await interaction.followup.send(f"✅ تم تسجيل التحذير في <#{self.warn_log_channel_id}>")
        if db[user_id]["count"] >= int(self.mod_config.get("max_warns", 3)):
            await interaction.followup.send(f"🚨 **تنبيه!** {target_user.mention} وصل للحد الأقصى! اختر عقوبة:", view=PunishmentView(target_user), ephemeral=False)
